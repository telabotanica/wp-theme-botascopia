...
function botascopia_header_walker_nav_menu_start_el($item_output, $item, $depth, $args) {
  if ( $args->theme_location === 'secondary' && $depth === 0 ) {
    $args->last_parent_el = $item;
  }
  return $item_output;
}
add_filter('walker_nav_menu_start_el', 'botascopia_header_walker_nav_menu_start_el', 10, 4);

// Remplacer à la volée les liens vers les catégories d'outils et de moyens de participer par des ancres
function botascopia_header_nav_menu_link_attributes($atts, $item, $args, $depth) {
...
add_filter('nav_menu_link_attributes', 'botascopia_header_nav_menu_link_attributes', 10, 4);
  ...
  public function end_lvl( &$output, $depth = 0, $args = array() ) {
    ...
    if ( $depth === 0 && isset( $args->last_parent_el ) && !empty( $args->last_parent_el->description ) ) {
      $output .= $indent . sprintf(
        ...
        get_botascopia_module('icon', ['icon' => 'info']),
