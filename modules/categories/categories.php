<?php function botascopia_module_categories($data) {
  ...
  $data->modifiers = botascopia_styleguide_modifiers_array('categories', $data->modifiers);

  // Fonction permettant d'afficher les catégories par défaut,
  // en se basant sur les catégories créées dans Wordpress
  if ( !function_exists( 'botascopia_module_categories_default' ) ) :
    function botascopia_module_categories_default() {
        ...
        $is_active = $category->term_id === $category_active; ?>
          <li class="categories-item<?php echo $is_active ? ' is-active' : '' ?>">
            <h3 class="categories-item-title"><a href="<?php echo esc_url( get_term_link( $category ) ) ?>" class="categories-item-link">
              <?php if ($icon) the_botascopia_module('icon', ['icon' => $icon]); ?>
  ...
  the_botascopia_module('title', [
    'title' => __('Catégories', 'botascopia'),
    'level' => 2,
  ...
  if ( !empty($data->items) ) :
    ...
    botascopia_module_categories_default();
